    ...
    <div className="card p-3 flex flex-col group">
      <div className="relative">
        ...
        >
          <IconHeart
            className={`w-4 h-4 ${
              isWishlisted(product.id) ? 'text-red-500' : 'text-slate-400'
            }`}
      ...
      <Link
        to={`/product/${product.id}`}
        className="mt-1 text-sm font-medium leading-snug line-clamp-2 hover:text-brand-700"
      >
        {product.name}
      </Link>
      ...
      <div className="mt-3">
        <button
          ...
          className="btn-outline w-full text-xs py-2 gap-1.5"
        >
          <IconCart className="w-4 h-4" />
          {product.stock === 0 ? 'Out of stock' : 'Add to Cart'}
        </button>
      </div>
      {/* Buy Now is only available on the single product page */}
    </div>
  )
}