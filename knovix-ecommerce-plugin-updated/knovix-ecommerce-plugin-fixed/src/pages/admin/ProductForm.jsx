...
const empty = { name: '', category: categories[0].id, price: '', mrp: '', stock: '', image: '', images: [], description: '', featured: false, bestSeller: false }
...
export default function ProductForm() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      getProduct(id)
        .then((p) => {
          setForm({ ...p, images: Array.isArray(p.images) ? p.images.filter((img) => img !== p.image) : [] })
          setLoading(false)
        })
  ...
  async function handleSubmit(e) {
    ...
    setError('')
    try {
      const galleryImages = (form.images || []).map((s) => s.trim()).filter(Boolean)
      const payload = {
        ...form,
        price: Number(form.price),
        mrp: Number(form.mrp),
        stock: Number(form.stock),
        images: [form.image, ...galleryImages].filter(Boolean),
      }
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="label mb-0">Additional Gallery Images</label>
            <button type="button" onClick={addGalleryImage} className="text-brand-700 text-xs font-medium">+ Add image</button>
          </div>
          <div className="space-y-2">
            ...
            {(form.images || []).length === 0 && (
              <p className="text-xs text-slate-400">No extra gallery images yet — the main Image URL above is shown alone.</p>
            )}
          </div>
        </div>
