...
function knovix_format_product($product) {
    ...
    // Full gallery: featured image first, then the WooCommerce product
    // gallery images. Falls back to just the featured image (or a
    // placeholder) when no gallery images are set, so the frontend's
    // thumbnail strip only appears when there's more than one photo.
    $images = [];
    if ($image_url) {
        $images[] = $image_url;
    }
