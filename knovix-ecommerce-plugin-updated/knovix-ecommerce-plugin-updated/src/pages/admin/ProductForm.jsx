...
const empty = { name: '', category: '', price: '', mrp: '', stock: '', image: '', description: '', featured: false, bestSeller: false }
...
export default function ProductForm() {
  ...
  useEffect(() => {
    getCategories().then((items) => {
      setCategories(items)
      // default a new product to the first real category once it's loaded
      setForm((f) => (!isEdit && !f.category && items[0] ? { ...f, category: items[0].id } : f))
    })
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <select required className="input" value={form.category} onChange={(e) => update('category', e.target.value)}>
              <option value="" disabled>{categories.length ? 'Select a category' : 'Loading categories…'}</option>
